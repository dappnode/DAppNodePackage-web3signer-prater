...
[[ "${CONSENSUS_CLIENT_PRATER}" == "teku" ]] && CERT_REQUEST="-k --cert-type P12 --cert /security/teku/cert/teku_client_keystore.p12:dappnode"
...
function send_dappmanager_notification() {
  curl -X POST -G 'http://my.dappnode/notification-send' --data-urlencode 'type=danger' --data-urlencode title="$CONSENSUS_CLIENT_PRATER is not available" --data-urlencode 'body=Make sure you select an available client in the web3signer at packages > web3signer prater > config > Prater Chain Consensus Layer Client'
}
...
function get_client_pubkeys() {
  ...
  http_code=${response: -3}
  content=$(echo "${response}" | head -c-4)
  response_middleware "$http_code" "$content" "$CONSENSUS_CLIENT_PRATER"
...
if [[ "${IS_BEACON_SYNCING}" == "true" ]]; then
  log info "beacon node is syncing, ${CONSENSUS_CLIENT_PRATER} API is not available, skipping public key comparison"
  exit 0
fi
